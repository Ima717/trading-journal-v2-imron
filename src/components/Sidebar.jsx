...
const Sidebar = () => {
  ...
  return (
    <motion.div
      ...
      className="h-screen bg-gray-100 text-gray-800 fixed z-20 shadow-lg"
    >
      <div className="flex items-center justify-between px-4 py-5">
        {!collapsed && <span className="text-xl font-bold tracking-wide text-gray-900">IMAI</span>}
      ...
      <div className="px-2">
        <Link to="/add-trade" className="w-full block mb-4 text-sm bg-blue-500 hover:bg-blue-600 text-white px-3 py-2 rounded text-center">
          ➕ {collapsed ? "" : "Add Trade"}
        </Link>
        <nav className="flex flex-col gap-1">
          {navItems.map((item) => (
            <Link
              key={item.name}
              to={item.path}
              className={`flex items-center gap-3 text-sm px-3 py-2 rounded hover:bg-gray-200 transition-all relative ${
      ...
      <div className="absolute bottom-4 w-full px-2">
        <Link
          to="/settings"
          className="flex items-center gap-3 text-sm px-3 py-2 rounded hover:bg-gray-200 transition-all"
        >
          <User size={20} />
        ...
        <button
          ...
          className="flex items-center gap-3 text-sm px-3 py-2 rounded hover:bg-gray-200 w-full text-left"
        >
          <LogOut size={20} />
